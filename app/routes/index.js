...
router.get('/productEdit/:id', (req, res) => {
  ...
  mysql.query(sql, condition, (err,result) => {
    if (err) {
      res.send(err);
    } else {
      ...
      mysql.query(sql, (err, products) => {
        ...
        if(err) {
          ...
          res.render('product', { data: result[0], products: products });
        }
      })
    }
  });
...
router.post('/searchProduct', (req,res) => {
  ...
  mysql.query(sql, req.body.barcode, (err,result) => {
    if (err) {
      res.send(err);
    } else {
      res.json(result);
...
router.get('/report', (req, res) => {
  var date = { from: '', to: '', products: [] };
  res.render('reportStock', date)
});
